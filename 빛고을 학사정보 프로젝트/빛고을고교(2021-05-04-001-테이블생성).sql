...
-- LEFT JOIN을 하여 import된 두 테이블간에 데이터 유효성 검증
...
SELECT 학번, 이름, SUM(점수) AS 총점, ROUND(AVG(점수),2) AS 평균
FROM view_성적정보
GROUP BY 학번, 이름
ORDER BY 학번;
...
CREATE SEQUENCE seq_subject_score
START WITH 1
INCREMENT BY 1;

DROP SEQUENCE seq_sbject_score;
