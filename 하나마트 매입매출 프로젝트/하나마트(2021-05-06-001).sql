...
-- 가치가 부가 되었다 라고 표현한다
-- 가치가 부가 된만큼 세금을 납부하도록 한다
-- 부가 가치세(Value Add Tax, VAT)
...
ORDER BY io_pname;
/*
매입매출 데이터로부터 상품정보 데이틀 데이터를 생성하기
1. 매입매출 데이터에서 상품명으로 그룹을 하고
2. 매입, 매출 구분에 따라 각각 매입단가, 매출단가를 가져오기
3. 매입과 매출에 0인 값이 있다
4. 매입단가가 0인 데이터는 매출데이터에서 임의로 생성하기
   매출단가의 80%를 매입단가로 하고, 부가세를 제외한 금액으로 계싼
   E2 항목의 값에 0.8를 곱하여 E2의 80% 가격이 되고
...
-- 상품정보 테이블
-- DEFAULT 속성
-- INSERT를 수행할 때 값이 지정되지 않으면
-- NULL값이 아니라 설정된 값으로 자동 추가된다. 
