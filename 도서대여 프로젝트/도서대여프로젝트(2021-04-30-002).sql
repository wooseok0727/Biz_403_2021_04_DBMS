...
-- books Table과 author, company table은 Relation(관계)가 있다
...
-- JOIN 된 데이터가 누락되거나 (null)이 출력되는 것은
-- 데이터에 문제가 발생한 것이다. 
-- "참조무결성"이 무너졌다
...
-- 다수의 table이 연관 Relation(관계)에 있을때
...
-- 코드가 변경되면 연관된 테이블의 모든 데이터의 코드값을 변경하거나
