...
-- Oracle Server와 Developer 사이에서 중간에
...
--      이때 사용하는 명령이 COMMIT
-- COMMIT 이 실행되어야만 저장소에 데이터가 확정된다
ROLLBACK;
COMMIT;
...
-- 옵션항목인 WHERE 절을 반드시 포함하자 사고를 방지하기 위해
-- 또한 특별하게 필요한 경우가 아니면
-- WHERE 절에 포함하는 조건은 PK(PRIMARY KEY) 칼럼을 기준으로 하자
...
-- DELETE 와 UPDATE 를 수행한 후 실수를 발견했을때
...
-- 영어점수, 수학점수 동점자가 있으면 국어점수 순으로 오름차순 정렬하라.
SELECT 학번, 영어, 수학, 국어 FROM view_score
WHERE 총점 < 200 
ORDER BY 학번, 영어 DESC, 수학 DESC, 국어;
...
-- 국어점수가 60 이상의 데이터
SELECT * FROM view_score
WHERE 총점 BETWEEN 150 AND 200 AND 국어 >= 60;
